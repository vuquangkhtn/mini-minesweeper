...
const GridMines = ({ mines, size }) => {
  ...
  for (let row = 0; row < size; row++) {
    ...
    for (let col = 0; col < size; col++) {
      ...
      const selected = selectedCells.includes(getKey(curCell));
      const isMine = selected && mines.includes(getKey(curCell));
  ...
  return (
    <StyledGrid>
      {grid.map(row => (
        <StyledRow>
          {row.map(cell => (
            <StyledCell selected={cell.selected} isMine={cell.isMine} onClick={handleCellSelected(cell)}>
...
function App() {
  ...
  useEffect(() => {
    if (levelType !== null) {
      const getMines = async ({ size, mines }) => {
        await fetch(`https://tiki-minesweeper.herokuapp.com/getMines?size=${size}&mines=${mines}`)
  ...
  return (
    <StyledApp>
      ...
      {mineList && <GridMines mines={mineList.map(item => getKey(item))} size={Level[levelType]?.size} />}
    </StyledApp>
  );
}
